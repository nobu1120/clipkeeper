...
### 3-5. プライバシーポリシーのホスティング ✅ 準備完了・実際の公開はユーザー承認待ち

[`store/PRIVACY_POLICY.md`](store/PRIVACY_POLICY.md)の内容（取得情報・保存場所・権限ごとの
正当性説明・第三者送信の有無・データ削除方法・実連絡先メールアドレスを含む）を、そのまま
GitHub Pagesで公開できる単一HTMLファイル [`docs/privacy-policy.html`](docs/privacy-policy.html)
として用意しました。**このファイルはまだ外部に公開されていません**。

このリポジトリは現時点で**GitHubリモートが未設定**です（`git remote`が空）。公開には、まず
GitHub上にリポジトリを作成してこのリポジトリをpushする必要があります。連携方法（GUI操作/
GitHub CLIの2通り）と、その後のpublic化・Pages設定・URL登録までの詳しい手順は
[`README.md`](README.md)「プライバシーポリシーの公開（GitHub Pages）」に記載しました。

→ リポジトリ作成・push・public化・GitHub Pages有効化はいずれも本リポジトリの外側で行う
外部公開行為のため、実行はユーザー自身の承認・操作待ちとし、今回は行っていません
（ユーザーの指示により保留中）。
...
確定しました。残るのは以下の通り、**このリポジトリの外側で行う実際の公開・提出アクション**の
みです（ユーザーの指示により意図的に保留中）。

1. **プライバシーポリシーの実際のホスティング**（[3-5](#3-5-プライバシーポリシーのホスティング-準備完了実際の公開はユーザー承認待ち)）
   - GitHubリポジトリの作成・public化・GitHub Pages有効化（または他のホスティング先の指定）の実行承認。
     連携方法は[`README.md`](README.md)「GitHubリポジトリの連携方法」に記載
2. **Chrome Web Store Developer Dashboardへの登録・$5登録料の支払い、および実際の審査提出**（保留中）
   - Googleアカウントでの支払い・申請は開発者ご本人が行う必要があります
     （[`store/SUBMISSION_GUIDE.md`](store/SUBMISSION_GUIDE.md)に手順を記載済み）
   - 上記1が確定し、実際に「申請してよい」というご承認をいただいた時点で、
     ストア掲載情報の最終セットをこちらで整えます
